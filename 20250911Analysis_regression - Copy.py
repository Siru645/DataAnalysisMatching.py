...
def identify_students_in_swaps(group_df, round_num):
    ...
    if num_indifferent == 3:
        ...
        indiff_idx = indifferent_mask.index(True)

        if len(non_indiff_indices) == 2:
            idx1, idx2 = non_indiff_indices
            indiff_assignment = assignments[indiff_idx]
            ...
            # Check if either non-indifferent student prefers the indifferent student's assignment
            student1_prefers_indiff = prefers_other_assignment(pref_lists[idx1], assignments[idx1], indiff_assignment)
            student2_prefers_indiff = prefers_other_assignment(pref_lists[idx2], assignments[idx2], indiff_assignment)

            if student1_prefers_indiff:
                    students_in_swaps.add(idx1)
            if student2_prefers_indiff:
                    students_in_swaps.add(idx2)
